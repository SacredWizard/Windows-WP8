...
namespace SoundMaster
{
    public partial class RecordAudio : PhoneApplicationPage
    {
        ...
        void recordAudioAppBar_Click(object sender, EventArgs e)
        {
            ...
            fileName.Completed += FileNameComplete;
            fileName.Show();
        }
        ...
        private void FileNameComplete(object sender, PopUpEventArgs<string, PopUpResult> e)
        {
            if (e.PopUpResult == PopUpResult.Ok)
            {
                ...
                //Save the list to CustomSounds to IsolatedStorage.ApplicationSettings
                ...
                // We'll need to modify sound model to retreive custom sounds from IsolatedStorage.ApplicationSettings
        ...
        {
            //Be Defensive Trust no one and Nothing
            if(buffer == null)
                throw new ArgumentNullException("Attempting to Save an Empty sound Buffer");

            //Clean out AudioPlayer's Hold on our audioStream
